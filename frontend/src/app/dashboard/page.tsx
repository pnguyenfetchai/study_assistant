...
export default function EducationalChat() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post("http://0.0.0.0:8000/submit", {
                request: input,
                response: "",
            });

            console.log("got it from frontend", response.data);


            const { type, data } = response.data;

            if (type === "text") {
                const assistantMessage: Message = { role: "assistant", content: data || "No response received." };
                ...
                setMessages((prev) => [...prev, { role: "assistant", content: "Unknown response type." }]);
            }
        } catch (error) {
